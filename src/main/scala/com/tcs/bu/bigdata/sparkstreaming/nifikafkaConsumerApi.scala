...
object nifikafkaConsumerApi {
  def main(args: Array[String]) {
    ...
    lines.foreachRDD { x =>
      ...
      val fst = x.first()
      val df = x.filter(x=>x!=fst).map(x=>x.split(",")).map(x=>(x(0).toInt,x(1).toString,x(2).toString,x(3).toString,x(4).toString,x(5).toString)).toDF("id","first_name","last_name","email","ip_address","mobile")
      df.show()
      ...
      blr.write.mode(SaveMode.Append).jdbc(ourl,"blrinfo123",oprop)
      del.write.mode(SaveMode.Append).jdbc(ourl,"delhiinfo123",oprop)*/
